...
Could you please tell me what you need, the preferred date/time, and your name?`;
  ...
  async function loadWorkspace() {
    try {
      ...
      if (!res.ok || !data?.workspace) {
        console.error("AI Training workspace API error:", data);
        setError(
          data?.error ||
            "Could not load your workspace. Please refresh and try again."
        );
        return;
      }

      const currentWorkspace = data.workspace as Workspace;
  ...
  async function saveTraining() {
    try {
      ...
      if (!res.ok || !data?.workspace) {
        console.error("AI Training save API error:", data);
        setError(data?.error || "Could not save AI training.");
        return;
      }

      const savedWorkspace = data.workspace as Workspace;

      setWorkspace(savedWorkspace);
      setBusinessName(savedWorkspace.business_name || cleanBusinessName);
      setBusinessType(savedWorkspace.business_type || cleanBusinessType);
      setMainLanguage(savedWorkspace.main_language || cleanMainLanguage);
      setAiJob(savedWorkspace.ai_job || cleanAiJob);
      setBusinessRules(savedWorkspace.business_rules || cleanBusinessRules);
