...
import { syncWebhookBodyToPrivateTables } from "@/lib/whatsapp/syncToPrivate";
...
function getOwnerPrivacyReply() {
  return "For company privacy, IÔÇÖm not able to share the owner or internal staff details. I can still help you with rentals, prices, location, licence rules, deposit, or booking details. ­ƒÿè";
}
...
async function sendInboundCustomerPushNotification({
  ...
  try {
    ...
    console.log("Ô£à Customer push notification sent:", {
      workspaceId: workspace.id,
      ownerUserId: workspace.owner_user_id,
      phone,
    });
...
export async function GET(req: NextRequest) {
  ...
  if (mode === "subscribe" && token === verifyToken && challenge) {
    console.log("Ô£à WhatsApp webhook verified successfully");
  ...
  console.error("ÔØî WhatsApp webhook verification failed", {
    mode,
    tokenReceived: token,
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      await syncWebhookBodyToPrivateTables(body);
    } catch (syncError) {
      console.error("Private table sync failed (non-fatal):", syncError);
    }
    ...
    if (!workspace?.id || !workspace?.owner_user_id) {
      console.error("ÔØî No Artipilot workspace found for webhook saving.");
    ...
    for (const entry of entries) {
      ...
      for (const change of changes) {
        ...
        console.log("­ƒôª WhatsApp webhook change parsed:", {
        ...
        for (const message of incomingMessages) {
          ...
          if (
            ...
            if (blockedIncomingSaved) {
              ...
              console.log("Ô£à Blocked contact inbound message saved:", {
                phone,
                messageType,
              });
            }
          ...
          if (needsHumanAttention) {
            ...
            console.log("­ƒÜ¿ Customer message triggered human attention:", {
              phone,
              reason: humanAttentionReason,
            });
          }
          ...
          if (ownerPrivacyQuestion) {
            try {
              ...
              console.log("Ô£à Owner privacy reply sent:", {
                phone,
                whatsappMessageId,
              });
            } catch (privacyError) {
          ...
          try {
            ...
            if (shouldMarkHumanAttentionFromText(cleanAiReply)) {
              ...
              console.log("­ƒÜ¿ AI reply triggered human attention:", {
                phone,
                reason:
                  "AI reply indicated that the team/human should confirm or continue this conversation.",
              });
            ...
            console.log("Ô£à AI WhatsApp reply sent successfully:", {
              phone,
              whatsappMessageId,
            });
          } catch (aiError) {
    ...
    // Meta may stop sending webhooks if we return 500 repeatedly.
    return NextResponse.json(
      {
        success: true,
        warning: error instanceof Error ? error.message : "Webhook handler failed",
      },
      { status: 200 }
    );
  }
